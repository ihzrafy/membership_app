    ...
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" action="{{ route('register.post') }}" method="POST">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                ...
                <div>
                    <label for="membership_type" class="sr-only">Membership Type</label>
                    <select id="membership_type" name="membership_type" required 
                            class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm @error('membership_type') border-red-500 @enderror">
                        <option value="">Choose Membership Type</option>
                        <option value="A" {{ old('membership_type') == 'A' ? 'selected' : '' }}>
                            Type A - Basic (3 articles & 3 videos)
                        </option>
                        <option value="B" {{ old('membership_type') == 'B' ? 'selected' : '' }}>
                            Type B - Standard (6 articles & 6 videos)
                        </option>
                        <option value="C" {{ old('membership_type') == 'C' ? 'selected' : '' }}>
                            Type C - Premium (12 articles & 12 videos)
                        </option>
                    </select>
